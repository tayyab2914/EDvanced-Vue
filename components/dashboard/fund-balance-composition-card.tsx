 ...
 * at the card's floor.
 *
 * THE BAR GEOMETRY IS COMPUTED, NOT TRACED — the mockup paints every row "$15.8M · 64.0%".
 * Each run is its true share of the total printed beneath it.
 */
...
export function FundBalanceCompositionCard({
  ...
  rows: CompositionRow[];
  stats: CapsuleStat[];
  /** Shown instead of the bars — no components committed, or the basis unavailable. */
  empty?: string;
}) {
  return (
    <OverviewPanel className="flex flex-col p-[18px]">
      <div className="flex items-start justify-between gap-[10px]">
        ...
        {controls && <div className="flex flex-none flex-col items-end gap-[4px]">{controls}</div>}
      </div>
          ...
          {rows.map((r) => {
            ...
            return (
              <li key={r.id}>
                ...
                {/* The 13px track: the share in its component colour, the rest hatched. */}
