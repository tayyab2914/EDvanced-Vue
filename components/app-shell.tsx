...
export function AppShell({
  ...
  return (
    <div className="flex min-h-screen">
      {/* SIDEBAR */}
      <aside className="sticky top-0 flex h-screen w-[250px] flex-none flex-col bg-navy">
        <div className="border-b border-white/[0.07] px-5 pb-4 pt-5">
          <div className="mb-4 flex items-center gap-2.5">
            <div className="flex h-[30px] w-[30px] items-center justify-center rounded-[7px] bg-brand text-[14px] font-bold text-white">
              {brand.trim()[0]}
            </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 flex h-[62px] flex-none items-center gap-4 border-b border-line bg-white px-6">
          <div className="min-w-0 flex-1">
          ...
          {contextTag && (
            <div className="hidden h-9 items-center gap-2 rounded-lg border border-line bg-panel px-3 text-[13px] font-medium text-ink-soft sm:flex">
              <span className="text-[12px] text-muted-2">Fiscal Year</span>
              <strong className="font-semibold">{contextTag}</strong>
            </div>
          )}
          <div className="relative flex h-9 w-9 items-center justify-center rounded-lg border border-line text-[#5b6a82]">
            <svg
              width="16"
              height="16"
              viewBox="0 0 24 24"
        ...
        <main className="mx-auto w-full max-w-[1200px] flex-1 px-7 py-7">
          {children}
        </main>
      </div>
    </div>
